...
def clean_pid(self, name):
    msgi = "---process to be handled: %s" % (name)
    self.config.options.logger.info(msgi)
    #call("/root/cleanup", name)
    pid_list = check_output(["pidof", name]).split()
    for pid in pid_list:
        #msgb = "---killing %s (%s)" % (name, pid)
        #self.config.options.logger.info(msgb)
        os.kill(int(pid), signal.SIGTERM)
        try:
            os.kill(int(pid), 0)
            #self.config.options.logger.info("QQ")
        except OSError as ex:
            continue
        #msga = "---%s (%s) is killed" % (name, pid)
        #self.config.options.logger.info(msga)
...
class Subprocess:
    ...
    def finish(self, pid, sts):
        ...
        if processname == "nginx":
            clean_pid(self, processname)
        #mymsg = "processname: %s" % (get_pid(processname))
        #self.config.options.logger.info(mymsg)
